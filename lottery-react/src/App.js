...
function App() {
	...
	const [doneCheckingForMetaMask, setDoneCheckingForMetaMask] =
		useState(false);
	...
	useEffect(() => {
		...
		if (connected) {
			async function handler() {
				const manager = await lotteryContract.current.methods
					.manager()
					.call();
				if (!cancelled) {
					setManager(manager);
	...
	const updatePlayersListAndBalance = async () => {
		const players = await lotteryContract.current.methods
			.getPlayers()
			.call();
			...
			{web3 === null && doneCheckingForMetaMask && (
				<div className="page-center">
					<div className="alert error">
						<h1 className="no-margin-top">Lottery Contract</h1>
						<p className="no-margin">
							MetaMask is required to run this app! Please install
				...
				<div className="page-center">
					<div className="alert error">
						...
						<p className="no-margin">
							...
							<strong>Rinkeby test network</strong> for Ether
			...
			{web3 !== null && !connected && isRinkebyChain && (
				<div className="page-center">
					<section className="card">
						<h1 className="no-margin-top">Lottery Contract</h1>
						<p>
							Want to try your luck in the lottery? Connect with
							MetaMask and start competing right away!
						</p>
						...
						<form onSubmit={onSubmit}>
							...
							<div>
								...
								<input
									...
									placeholder="Minimum of > .01 ether"
								/>{" "}
								<button
									className="btn primaryBtn"
									type="submit"
